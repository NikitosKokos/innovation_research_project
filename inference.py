...
# Suppress transformers warnings about missing sampling_rate (we handle it explicitly)
warnings.filterwarnings('ignore', message='.*sampling_rate.*')
# Suppress harmless model loading warnings about missing F0 weights (expected when F0 conditioning is disabled)
warnings.filterwarnings('ignore', message='.*Skipped loading some keys.*')
